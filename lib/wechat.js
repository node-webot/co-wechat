...
var wechat = function (config) {
  ...
  if (typeof config !== 'object' || !config.path || !config.token) {
    throw new Error('please check your config, path and token are required');
  }
...
wechat.prototype.middlewarify = function () {
  var that = this;
  ...
  return function* (next) {
    ...
    if (method === 'GET') {
      ...
      // 钩子函数，即可用于msgType方法执行前的预处理，也可直接使用返回结果
      var hookMethod = that.getHandler('all');
      if (hookMethod) {
        yield* hookMethod.call(this);
      }

      // 根据消息类型调用相应的Handler
      var type = formated.MsgType;
      var methodHandler = that.getHandler(type);
      if (methodHandler) {
        yield* methodHandler.call(this);
      }
      yield* next;

      this.body = reply(this.body, formated.FromUserName, formated.ToUserName);
 ...
 */
wechat.prototype.setHandler = function (type, fn) {
  this.handlers[type] = fn;
  return this;
};

['all', 'text', 'image', 'voice', 'video', 'location', 'link', 'event'].forEach(function (method) {
  wechat.prototype[method] = function (fn) {
 ...
 */
wechat.prototype.getHandler = function (type) {
