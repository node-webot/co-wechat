  ...
  return function* (next) {
    ...
    if (method === 'GET') {
      ...
      if (this.sessionStore) {
        this.openid = formated.FromUserName + ':' + formated.ToUserName;
        this.wxsession = yield this.sessionStore.get(this.openid);
        if (!this.wxsession) {
          this.wxsession = {};
      ...
      if (this.sessionStore) {
        if (!this.wxsession) {
          if (this.openid) {
            yield this.sessionStore.destroy(this.openid);
          }
        } else {
          yield this.sessionStore.set(this.openid, this.wxsession);
        }
      }

      this.body = reply(this.body, formated.ToUserName, formated.FromUserName);
      if (encrypted) {
        var wrap = {};
        wrap.encrypt = handler.cryptor.encrypt(this.body);
