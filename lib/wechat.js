  ...
  return async (ctx, next) => {
    ...
    if (method === 'GET') {
      ...
      if (this.sessionStore) {
        ...
        ctx.wxsession = await ctx.sessionStore.get(ctx.wxSessionId);
        if (!this.wxsession) {
          ctx.wxsession = {};
      ...
      if (ctx.sessionStore) {
        if (!ctx.wxsession) {
          if (ctx.wxSessionId) {
            await ctx.sessionStore.destroy(ctx.wxSessionId);
          }
        } else {
          await ctx.sessionStore.set(ctx.wxSessionId, ctx.wxsession);
        }
      }
